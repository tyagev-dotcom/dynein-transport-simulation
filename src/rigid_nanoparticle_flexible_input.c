...
// Spread motors uniformly over the nanoparticle surface and attach the first
// one
void spreadMotors(struct MotorData *Motors, unsigned int *seed) {
  double phi, teta;
...
struct BalanceMovment GradientDescent(struct MotorData Motors) {
  ...
  double prevEnergy, currentEnergy, momentum_x = 0, momentum_y = 0,
                                    momentum_z = 0;
...
int main(void) {
  ...
  FILE *infile = fopen("Input-NP-flex.txt", "r");
  if (infile == NULL) {
    printf("Error: Could not open Input-NP-flex.txt\n");
    return 1;
  }
  if (fscanf(infile, "%d", &numMotors) != 1 ||
      fscanf(infile, "%d", &numTrials) != 1 ||
      fscanf(infile, "%lf", &radiusNC) != 1 ||
      fscanf(infile, "%lf", &dyenin_pers_length) != 1 ||
      fscanf(infile, "%lf", &PolymerLength) != 1) {
    printf("Error: Failed to read parameters from Input-NP-flex.txt\n");
    fclose(infile);
    return 1;
  }
  fclose(infile);
  ...
  // save data
  FILE *file = fopen("output-NP-flex.txt", "w"); // Open the file in write mode
  if (file == NULL) {
